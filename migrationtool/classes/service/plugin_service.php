<?php
//plugin verifier, checks for other plugins installed for quizzes, if there is a plugin from source instance but not in destination instance it will not let the migration to start.
namespace local_migrationtool\service;

defined('MOODLE_INTERNAL') || die();

class plugin_service {
    public function collect_for_courses(array $courseids, array $scope): array {
        global $DB;
        ...
        foreach ($courseids as $courseid) {
            ...
            if ($scope['question_bank']) {
                foreach ($this->get_question_types($courseid) as $qtype) {
    ...
    public function compare(array $sourcecomponents): array {
        ...
        foreach ($sourcecomponents as $source) {
            ...
            if ($sourceversion !== null && $targetversion !== null &&
                    version_compare((string)$targetversion, (string)$sourceversion, '<')) {
                $status = 'error';
                $message = 'The target component version is older than the source version.';
            } else if ($sourceversion !== null && $targetversion !== null &&
                    (string)$targetversion !== (string)$sourceversion) {
            ...
            if (($source['source'] ?? null) !== null && $target->source !== ($source['source'] ?? null) &&
                    $status === 'ok') {
                $status = 'warning';
    ...
    private function get_question_types(int $courseid): array {
        global $DB;

        $qtypes = [];

        //quiz questions
        $usedqtypes = $DB->get_fieldset_sql(
        ...
        foreach ($usedqtypes as $qtype) {
            $qtypes[$qtype] = true;
        }

        //questions in course or activity contexts.
        $bankqtypes = $DB->get_fieldset_sql(
            "SELECT DISTINCT q.qtype
               FROM {question} q
        ...
        foreach ($bankqtypes as $qtype) {
            $qtypes[$qtype] = true;
        }

        ksort($qtypes);
        return array_keys($qtypes);
    }
